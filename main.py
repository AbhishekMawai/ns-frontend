...
def fetch_last_5_entries():
    response = requests.get("https://nsbackend-rai571e3.b4a.run/last_entries/")
...
def main():
    ...
    if st.button("Add Entry"):
        response = requests.post(f"https://nsbackend-rai571e3.b4a.run/add_entry/?date={date}&close_price={close_price}")
        if response.status_code == 200:
    ...
    if st.button("Delete"):
        response = requests.post(f"https://nsbackend-rai571e3.b4a.run/delete_entry/?date={delete_date}")
        if response.status_code == 200:
            message = response.json().get("message", "No message returned from server.")
            st.write(message)
